...
// that must be paired by color for sale. Given an array of integers 
...
function sockMerchant(n, ar) {
  let unpairedSocks = new Set();
  let pairs = 0; // keep track of pairs
  for (let sock of ar) {
      if (unpairedSocks.has(sock)) {
          unpairedSocks.delete(sock);
          pairs++;
      } else {
